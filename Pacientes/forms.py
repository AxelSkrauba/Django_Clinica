...
class ConsultationsForm(forms.ModelForm):
    ...
    date = forms.DateField(label="Fecha de consulta",widget=forms.SelectDateWidget(years=range(today.year,today.year+2)))
    time = forms.TimeField(label="Hora de consulta",help_text="Ejemplo: 8:00",widget=widgets.AdminTimeWidget)
    ...
    def clean(self, *args, **kwargs):
        ...
        if date is not None:
            if date < self.today:
                self.add_error('date', 'La consulta no puede especificarse en una fecha "pasada".')
        ...
        if doctor is not None and time is not None:
            for consultation in query:
                if doctor == consultation.doctor and time == consultation.time and date == consultation.date:
                    self.add_error('doctor', 'El profesional ya cuenta con un turno asignado en el horario definido.')
